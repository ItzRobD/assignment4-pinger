...
def ping(host, timeout=1):
    ...
    for i in range(0, 4):
        ...
        if delay == "Request timed out.":
            df_to_add = pd.DataFrame({'bytes': [0], 'rtt': [delay], 'ttl': [0]})
        else:
            df_to_add = pd.DataFrame({'bytes': [64], 'rtt': [delay], 'ttl': [64]})
        response = pd.concat([response, df_to_add], ignore_index=True)
        print(delay)
        time.sleep(1)
